...
from chat_sidebar import render_rag_chat
...
# 1. 建立永久存储文件夹
UPLOAD_DIR = "uploaded_monthly_data"
...
with st.sidebar:
    ...
    if uploaded_files:
        ...
        # 界面提示
        if added_count > 0:
            st.success(f"✅ Successfully saved {added_count} new file(s)!")
        if skipped_files:
            st.info(f"⏭️ Skipped {len(skipped_files)} file(s) (Already exist).")

    # 4. 无论是否上传新文件，永远读取文件夹里的所有历史文件
    all_local_files = [os.path.join(UPLOAD_DIR, f) for f in os.listdir(UPLOAD_DIR) if f.endswith('.csv')]
    
    # 5. 合并成总表 (df)
    if all_local_files:
        st.success(f"📊 Database active: **{len(all_local_files)}** historical file(s).")
        try:
            # 自动读取所有文件并上下拼接合并
            df_list = [pd.read_csv(fp) for fp in all_local_files]
            df = pd.concat(df_list, ignore_index=True)
            
            with st.expander("📥 Download Merged Raw Data"):
                csv_data = df.to_csv(index=False, encoding='utf-8-sig')
                st.download_button(label="Click here to download", data=csv_data, file_name="merged_historical.csv", mime="text/csv")
...
st.markdown("""
<style>
/* 1. 放大 Expander (Affiliates Assistant) 的标题字体 */
[data-testid="stExpander"] details summary p {
    font-size: 20px !important;  /* 字体大小，可自行调整 */
    font-weight: 700 !important; /* 加粗 */
    color: #1E1E1E !important;   /* 颜色更深一些 */
}

/* 2. 让聊天输入框随文字自动拉伸得更大 */
[data-testid="stChatInput"] textarea {
    min-height: 50px !important;   /* 初始最低高度稍微大一点，方便点按 */
    max-height: 400px !important;  /* 允许自动拉伸到非常大（原版默认很小），发送后会自动缩回 */
}
</style>
...
# 1. 在页面右上角（或左上角）新增一个极其优雅的现代开关 (Toggle)
show_ai = st.toggle("✨ Show/Hide AI Assistant", value=True)

# 2. 动态分配布局 (核心魔法)
if show_ai:
    # 当开关打开时，划分为 6:1 的双列模式
    col_main, col_right = st.columns([4, 1])
else:
    # 当开关关闭时，不再分列，直接创建一个独占 100% 全宽的容器
    col_main = st.container()

# 3. --- 右侧边栏：AI 助手 ---
# 只有在开关打开的状态下，才去渲染右侧的框
if show_ai:
    with col_right:
        with st.expander("Affiliates Assistant", expanded=True):
            # 调用刚刚引入的函数，并把当前环境的 db_manager 传进去
            render_rag_chat(db_manager)




# --- 左侧主面板：原有的 Dashboard 内容 ---
with col_main:
    ...
    if 'df' in locals() and not df.empty:
        try:
            ...
            with col1:
                ...
                # 默认选中的索引设为最后一位（即最大/最新年份）
                default_year_idx = len(years) - 1 if len(years) > 0 else 0
                selected_year = st.selectbox("📌 Choose a Year", years, index=default_year_idx)
                
            with col2:
                # 根据选中的年份，提取对应的所有月份，并从小到大排序
                months = sorted(df[df['Year'] == selected_year]['Month'].dropna().unique().tolist())
                # 默认选中的索引设为最后一位（即最大/最新月份）
                default_month_idx = len(months) - 1 if len(months) > 0 else 0
